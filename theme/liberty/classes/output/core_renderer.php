...
class core_renderer extends \core_renderer {
    ...
    public function full_header() {
        ...
        $html .= html_writer::start_div('pull-xs-left context-header');
    ...
    public function navbar() {
        ...
        foreach($items as $item){
            if(strstr($item->action, 'course/view.php') && !strstr($item->action, 'section=')){
                $coursename = shorten_coursename($item->text);
                $item->text = '<span class="coursename">'.$coursename.'</span>';
            }
        }
    ...
    protected function render_context_header(\context_header $contextheader) {
        ...
        if (!empty($PAGE->layout_options['course-header-info']) && empty($_GET['section'])){
            global $DB;
            $role = $DB->get_record('role', array('shortname' => 'editingteacher'));
            $context = get_context_instance(CONTEXT_COURSE, $PAGE->course->id);
            $teachers = get_role_users($role->id, $context);
            if(count($teachers)>0 && count($teachers)<3){
                $html .= '<span class="teacher-pictures">';
                foreach($teachers as $teacher){
                    $userpicture = new user_picture($teacher);
                    $userpicture->size = 124; // Size f1.
                    $image = $userpicture->get_url($PAGE);
                    $html .= '<img class="teacherpicture" src="'.$image.'" />';
                }
                $html .= '</span>';
            }
        }
        ...
        // Headings.
        if (!isset($contextheader->heading) || !empty($_GET['id'])) {
            $headings = $this->heading($this->page->heading, $contextheader->headinglevel);
        } else {
            $headings = $this->heading('Welcome back, '.$contextheader->heading.'!', $contextheader->headinglevel);
        }
        $html .= \html_writer::start_div('page-header');
        $html .= \html_writer::tag('div', $headings, array('class' => 'page-header-headings'));

        $pageheadingbutton = $this->page_heading_button();
        if (empty($PAGE->layout_options['nonavbar']) && (empty($PAGE->layout_options['course-header-info']) || !empty($_GET['section']))) {
            $html .= html_writer::start_div('clearfix w-100 pull-xs-left', array('id' => 'page-navbar'));
            $html .= html_writer::tag('div', $this->navbar(), array('class' => 'breadcrumb-nav'));
            $html .= html_writer::div($pageheadingbutton, 'breadcrumb-button pull-xs-right');
            $html .= html_writer::end_div();

        // If it is the course homepage
        } else if (!empty($PAGE->layout_options['course-header-info']) && empty($_GET['section'])){
            $html .= html_writer::start_div('clearfix w-100 pull-xs-left', array('id' => 'page-navbar'));
            $courseInfo = '<span class="courseinfo">';
            $courseInfo .= '<span class="coursename">';
            $courseInfo .= shorten_coursename($PAGE->course->shortname);
            $courseInfo .= '</span>';
            $courseInfo .= '<span class="courseteachers">';
            if(count($teachers)>0 && count($teachers)<3){
                $counter = 0;
                foreach($teachers as $teacher){
                    $counter++;
                    ...
                    $teacherURL = new moodle_url('/user/profile.php?id='.$teacher->id);
                    $courseInfo .= '<a href="'.$teacherURL.'">'.$teacher->firstname.' '.$teacher->lastname.'</a>';
                }
            } else if (count($teachers) >= 3) {
                $viewTeachersURL = new moodle_url('/user/index.php?id='.$PAGE->course->id.'&roleid=3');
                $courseInfo .= '<a href="'.$viewTeachersURL.'" class="view-professors">View Professors</a>';
            }
            $courseInfo .= '</span>';
            $courseInfo .= '</span>';
            $html .= html_writer::span($courseInfo, 'header-button-title');
            $html .= html_writer::end_div();
    ...
    public function get_compact_logo_url($maxwidth = 100, $maxheight = 100) {
        global $CFG;
        return $CFG->httpswwwroot.'/theme/liberty/pix/lu-courses-logo-mobile.svg#Page-1';
    ...
    public function get_logo_url($maxwidth = null, $maxheight = 200) {
        global $CFG;
        return $CFG->httpswwwroot.'/theme/liberty/pix/lu-courses-logo.svg#Matching-FCI';
